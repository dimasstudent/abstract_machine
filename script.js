 // State Variables
      ...
      function getNodeLayout(statesList) {
        ...
        statesList.forEach((state, index) => {
          if (nodePositions[state]) {
            ...
            const avgX =
              Object.values(nodePositions).reduce(
                (sum, pos) => sum + pos.x,
                0
      ...
      simulateBtn.addEventListener("click", () => {
        ...
        if (simResult.error) {
          ...
          result.textContent = `String DITERIMA oleh mesin ${
            isDfa ? "DFA" : "NFA"
          }.`;
      ...
      function highlightPath(history, isDfa) {
        ...
        // Reset semua warna
        nodesData.update(
          nodesData.getIds().map((id) => ({ id, color: "#97c2fc" }))
        );
        edgesData.update(
          edgesData.getIds().map((id) => ({ id, color: "#848484" }))
        );

        // Jika DFA, highlight jalur step-by-step
        if (isDfa) {
          history.forEach((step, index) => {
            if (index > 0) {
              ...
              if (edgeId) {
                edgesData.update({ id: edgeId, color: { color: "#ff0000" } });
              }
              nodesData.update({ id: step.state, color: "#ffa500" }); // kuning oranye
            } else {
              nodesData.update({ id: step.state, color: "#a3d9a5" }); // hijau muda
            }
          });
        } else {
          // Jika NFA, highlight semua state aktif
          history.forEach((step) => {
            if (step.states) {
              step.states.forEach((state) => {
                nodesData.update({ id: state, color: "#ffa500" });
              });
            }
          });
        }
